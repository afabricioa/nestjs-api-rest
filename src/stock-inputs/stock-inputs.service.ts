...
export class StockInputsService {
  ...
  async create(createStockInputDto: CreateStockInputDto) {
    ...
    // esse transaction serve para garantir que vai persistir no banco apenas se as duas modificações
    // forem bem feitas com sucesso
  ...
  async findOne(id: number) {
    try {
      ...
      if(error.code === "P2025"){
        throw new NotFoundError(`StockInput com o ID ${id} não encontrado`);
      }
    }
  }
}
